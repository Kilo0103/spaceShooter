...
public class PlayCtrl_3 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            anim.CrossFade(runF.name, 0.3f); //전진
        }
        else if (v <= -0.0001f)
        {
            //anim.CrossFade(playerAnim.runF.name, 0.3f); >> anim.CrossFade(runB....); 생략이 진행됨
            anim.CrossFade(runB.name, 0.3f); //후진
        }
        else if (h >= 0.0001f)
        {
            anim.CrossFade(runR.name, 0.3f); //오른쪽
        }
        else if (h <= -0.0001f)
        {
            anim.CrossFade(runL.name, 0.3f); //왼쪽
        }
        else
        {
            anim.CrossFade(idle.name, 0.3f); //대기
        }
